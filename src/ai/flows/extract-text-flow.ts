...
/**
 * @fileOverview Extracts text from an image using generative AI.
 *
...
const ExtractTextFromImageOutputSchema = z.object({
  extractedText: z
    .string()
    .describe('The text extracted from the document image.'),
  confidence: z
    .number()
    .describe(
      'A confidence score between 0 and 100 for the OCR extraction.'
    ),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert OCR system. Extract all text from the following document image.
Also provide a confidence score for the extraction quality.
